...
from helpers.gemini import classify_text
...
st.title(f"Phishing Detection System")
st.markdown("---")
...
with st.sidebar:
    ...
    st.text_input(
        "API Key", type="password", help="Enter your Gemini API key", key="api"
    )
    ...
    timeout = st.slider(
        label="Tesseract OCR timeout [sec]",
        min_value=1,
        max_value=60,
        step=1,
        key="timeout",
    ...
    cGrayscale = st.checkbox(label="Grayscale", key="cGrayscale")
    cDenoising = st.checkbox(label="Denoising", key="cDenoising")
    cDenoisingStrength = st.slider(
        label="Denoising Strength",
        min_value=1,
        max_value=40,
        step=1,
        key="cDenoisingStrength",
    )
    cThresholding = st.checkbox(label="Thresholding", key="cThresholding")
    ...
    cRotate90 = st.checkbox(label="Rotate in 90° steps", key="cRotate90")
    angle90 = st.slider(
    ...
    cRotateFree = st.checkbox(label="Rotate in free degrees", key="cRotateFree")
    angle = st.slider(
        "Rotate freely [Degree]", min_value=-180, max_value=180, step=1, key="angle"
    )
    st.markdown(
        """---
# About
Streamlit app to detect phishing websites using machine learning models.
## GitHub
<https://github.com/DeathEaterSam/Phishing-Detection-System>
""",
...
custom_oem_psm_config = tesseract.get_tesseract_config(
    oem_index=oem_index, psm_index=psm_index
)
...
with col_upload_1:
    ...
    if st.session_state.uploaded_file is None:
        ...
        if st.session_state.uploaded_file.name.lower().endswith(".pdf"):
            ...
            st.session_state.raw_image, error = pdfimage.pdftoimage(
                pdf_file=st.session_state.uploaded_file, page=page
            )
            if error:
                st.error(error)
                st.stop()
        ...
        # else uploaded file is image file
            ...
            try:
                ...
                st.session_state.raw_image = opencv.load_image(
        ...
        try:
            ...
            st.error(str(e))
            st.stop()
        st.session_state.preview_processed_image = st.toggle(
            "Preview Preprocessed Image", value=True
        )
...
with col_upload_2:
    ...
    if language_short not in installed_languages:
        st.error(
            f'Selected language "{language}" is not installed. Please install language data.'
        )
        st.stop()
...
if st.session_state.uploaded_file is not None:
    ...
    with col1:
        if st.session_state.preview_processed_image:
            st.subheader("Preview after Preprocessing :eyes:")
            if st.session_state.image is not None:
                image = opencv.convert_to_rgb(
                    st.session_state.image
                )  # convert BGR to RGB
                st.image(
                    image,
                    caption="Image Preview after Preprocessing",
                    use_column_width=True,
                )
        else:
            st.subheader("Preview after Upload :eye:")
            if st.session_state.raw_image is not None:
                raw_image = opencv.convert_to_rgb(
                    st.session_state.raw_image
                )  # convert BGR to RGB
                st.image(
                    raw_image,
                    caption="Image Preview after Upload",
                    use_column_width=True,
                )

    with col2:
        if st.session_state.prediction:
            # st.text_area(label="Output", value=st.session_state.prediction, height=500)
